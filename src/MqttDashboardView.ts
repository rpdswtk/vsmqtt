...
export class MqttDashboardView {
    public static readonly viewType = "mqtt-dashboard";
    ...
    private static _openViews: Map<string, MqttDashboardView> = new Map<string, MqttDashboardView>();
    ...
    public static createOrShow(extensionUri: vscode.Uri, brokerConfig: MqttBrokerConfig) {
        ...
        let existingView = MqttDashboardView._openViews.get(brokerConfig.name);
        if (existingView) {
        ...
        MqttDashboardView._openViews.set(brokerConfig.name, new MqttDashboardView(panel, extensionUri, brokerConfig));
    }

    public static kill(brokerConfig: MqttBrokerConfig) {
        MqttDashboardView._openViews.get(brokerConfig.name)?.dispose();
    }
    ...
    public static reveal(brokerConfig: MqttBrokerConfig) {
        MqttDashboardView._openViews.get(brokerConfig.name)?._panel.reveal();
    }
    ...
    public dispose() {
        MqttDashboardView._openViews.delete(this.brokerConfig.name);
    ...
    private _getHtmlForWebview(webview: vscode.Webview) {
        ...
        const scriptUri = webview.asWebviewUri(
            vscode.Uri.joinPath(this._extensionUri, "out", "compiled/MqttDashboardPanel.js")
        );

        const stylesPageUri = webview.asWebviewUri(
            vscode.Uri.joinPath(this._extensionUri, "out", "compiled/MqttDashboardPanel.css")
        );
